...
async def create_meme(image_url: str, text0: str, text1: str) -> Any:
    try:
        ...
        text0_width, text0_height = (
            text0_bbox[2] - text0_bbox[0],
            text0_bbox[3] - text0_bbox[1],
        )

        text1_bbox = draw.textbbox((0, 0), text1, font=font)
        text1_width, text1_height = (
            text1_bbox[2] - text1_bbox[0],
            text1_bbox[3] - text1_bbox[1],
        )

        text0_position: Tuple[int, int] = ((image.width - text0_width) // 2, 0)
        text1_position: Tuple[int, int] = (
            (image.width - text1_width) // 2,
            image.height - text1_height - 16,
        )
        ...
        image.save(
            os.path.join(os.getcwd(), f"{random_filename}.jpeg"),
            format="JPEG",
            encoding="utf-8",
        )
